...
type QREmailContent = {
  ...
  body: string;
  fields: { show: string; date: string; tickets: string; total: string; reservationId: string };
  qrHint: string;
  studentNote: string;
  footer: string;
  subfooter: string;
...
async function sendQREmail(
  ...
  await transporter.sendMail({
    ...
    subject: c.subject,
    html,
    attachments: [{ filename: "qr.png", content: qrBuffer, cid: "qr" }],
  });
}

const confirmedContent: Record<Language, QREmailContent> = {
  es: {
...
export async function sendPaymentConfirmed(
  ...
  return sendQREmail(
    confirmedContent[language] ?? confirmedContent.es,
    email, name, show, tickets, amount, reservationId, mongoId, isStudent
  );
}

// ES strings differ from confirmedContent; EN and DE are identical.
const ticketConfirmedContent: Record<Language, QREmailContent> = {
  es: {
    subject: "Ticket — Los Mutantes",
    heading: "¡Tickets listos!",
    greeting: (name: string) => `Hola <strong>${name}</strong>,`,
    body: "Tus tickets ya están listos. Muestra el código QR en la entrada el día de la función.",
    fields: {
      show: "Función",
      date: "Fecha",
      tickets: "Entradas",
  ...
  en: {
    subject: "Your tickets — Los Mutantes",
    heading: "Your tickets are ready!",
    greeting: (name: string) => `Hi <strong>${name}</strong>,`,
    body: "Your tickets are ready. Show the QR code at the door on the day of the show.",
    fields: {
      show: "Show",
      date: "Date",
      tickets: "Tickets",
  ...
  de: {
    subject: "Deine Tickets — Los Mutantes",
    heading: "Deine Tickets sind bereit!",
    greeting: (name: string) => `Hallo <strong>${name}</strong>,`,
    body: "Deine Tickets sind fertig. Zeige den QR-Code am Eingang am Vorstellungstag.",
    fields: {
      show: "Vorstellung",
      date: "Datum",
      tickets: "Tickets",
...
export async function sendTicketConfirmation(
  ...
  return sendQREmail(
    ticketConfirmedContent[language] ?? ticketConfirmedContent.es,
    email, name, show, tickets, amount, reservationId, mongoId, isStudent
  );
}